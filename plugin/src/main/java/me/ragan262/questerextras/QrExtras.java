...
import me.ragan262.questerextras.listeners.EpicBossRecodedListener;
...
public class QrExtras extends JavaPlugin {
	...
	public void onEnable() {
		...
		if(getServer().getPluginManager().getPlugin("EpicBoss Gold Edition") != null) {
			getServer().getPluginManager().registerEvents(new EpicBossGoldListener(), this);
			getLogger().info("EpicBoss Gold Edition found and hooked.");
		}
		else if(getServer().getPluginManager().getPlugin("EpicBossRecoded") != null) {
			getServer().getPluginManager().registerEvents(new EpicBossRecodedListener(), this);
			getLogger().info("EpicBossRecoded found and hooked.");
		}
		// reset disable
		disable = false;
	}
